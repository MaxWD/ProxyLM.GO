...
| ID     | Требование |
|--------|------------|
| FR-69  | Daemon ДОЛЖЕН отдавать браузерный dashboard из статически встроенных (в бинарник на этапе компиляции) ассетов по пути `GET /ui/*`, с редиректом `GET /ui` → `/ui/`. Ответы под `/ui/*` ДОЛЖНЫ содержать `Cache-Control: no-cache`. Путь `/ui/*` НЕ ДОЛЖЕН требовать аутентификации на уровне HTTP — сами по себе статические ассеты не несут секретов и живых данных; все живые данные получаются собственным WebSocket-соединением страницы к `/admin/stream`, аутентифицируемым по FR-70. |
...
В базовой линии v0.9.3 явно **не** реализуются. Примечание: строго read-only встроенный Web UI dashboard появился в v0.14.0 (§3.14, FR-69..FR-71) — это не полноценный read-write веб-интерфейс управления, и перечисленные ниже пункты остаются out-of-scope независимо от него.
