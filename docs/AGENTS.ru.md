...
- Поддерживать `docs/FUTURE.ru.md` — «парковка идей». При каждом релизе/значимом изменении ревизировать список: убирать реализованное, фиксировать новые идеи, всплывшие в ходе работы. Из FUTURE.ru.md **ничего не реализуется самим агентом или Claude Code** — пункты выполняются только по явному запросу пользователя (см. §FUTURE-RULE в CLAUDE.md).
