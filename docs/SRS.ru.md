...
| FR-41  | Daemon ДОЛЖЕН собирать все наблюдения завершённых запросов по ключам `(server_name, model, endpoint)` в памяти и вычислять параметры линейной модели `loaded·t_load + b·k_in + c·k_out ≈ t_all` методом **ridge-регрессии** (`(X^T X + λI)θ = X^T y`, λ = 1e-4) — 3×3 нормальные уравнения; fallback 2×2 если нет reload-точек или матрица вырождена. Минимум для валидной оценки — 3 наблюдения (`perfMinSamples = 3`). Endpoint стал частью ключа в v0.10.0: `/v1/chat/completions`, `/v1/embeddings` и т. п. отслеживаются отдельно, чтобы смешивание разных типов запросов не искажало регрессию. Поля `perf_ok`, `t_load_ms`, `tok_in_per_sec`, `tok_out_per_sec`, `r_squared`, `fit_quality` публикуются в `ServerState`; разрез per-(model,endpoint) (с `t_load_ci` / `k_in_ci` / `k_out_ci` — half-width 95% доверительного интервала, появились в v0.10.0) публикуется в `ServerState.per_model_stats[]`. TUI ДОЛЖЕН показывать header-метрики в строке HeaderBar в формате `t_load · ↓tok_in/s · ↑tok_out/s`; при `perf_ok = false` или пустом `current_model` строка метрик пустая. В Info-pane для сервера TUI ДОЛЖЕН выводить строки per `(model, endpoint)` с показом R² и CI коэффициентов; строки с `fit_quality = "degraded"` (R² < 0.70) ДОЛЖНЫ визуально подсвечиваться. Поля `tokens_per_sec` и `ttft_ms` **удалены**. |
...
### 3.12. Персистентность perf-статистики (v0.14.0)

| ID     | Требование |
|--------|------------|
| FR-66  | Каждое наблюдение, принятое `PerfTracker.Record` (то есть прошедшее валидацию), ДОЛЖНО дополнительно персиститься в таблицу SQLite `perf_observations` (схема — в `ARCHITECTURE.ru.md` §10) через асинхронный неблокирующий writer: горячий путь планировщика НИКОГДА не должен блокироваться этой записью, а при заполненной очереди writer'а наблюдение ДОЛЖНО молча дропаться (Debug-лог), а не создавать backpressure. При старте daemon'а, до начала приёма HTTP-запросов, ранее сохранённые наблюдения ДОЛЖНЫ быть загружены и массово восстановлены в `PerfTracker` по каждому ключу `(server, model, endpoint)` в том же FIFO-порядке, в каком они были записаны, без повторного запуска персистентности для восстановленных точек. Кольцевой буфер на диске на каждый ключ ДОЛЖЕН быть ограничен тем же размером, что и in-memory cap (`core.PerfMaxObservations`, 1000), через периодический trim (при старте и не реже раза в час), независимо от `storage.history_retention_days`. У этой функции нет флага конфига — она всегда включена. |

### 3.13. Валидация здоровья бэкенда (v0.14.0)

| ID     | Требование |
|--------|------------|
| FR-67  | Ответ `GET /v1/models` с кодом 2xx ДОЛЖЕН трактоваться как healthy с нулём известных моделей, если тело ответа — синтаксически валидный OpenAI-список моделей с пустым массивом `data` (`{"data": []}`) или пустой верхнеуровневый массив (`[]`); в этом случае прокси ДОЛЖЕН логировать WARN («server healthy but reports zero models»). Ответ 2xx, тело которого не OpenAI-совместимо — объекты в `data` без непустого строкового `id`, либо любая другая форма (HTML, произвольный JSON) — НЕ ДОЛЖЕН трактоваться как healthy: прокси ДОЛЖЕН пометить сервер `unhealthy` и залогировать актуализирующий WARN, указывающий на несовпадение формы ответа и рекомендующий проверить `backends[].url`. Это применяется и к initial healthcheck (FR-49), и к периодическим опросам discovery (FR-24). |
| FR-68  | Для сервера с непустым явным списком `backends[].models` ошибка валидации формы ответа (по FR-67), возникшая при **периодическом** опросе discovery, ДОЛЖНА трактоваться как успешная проверка живости, а не как неудачный опрос: эндпоинт ответил, а явный список моделей остаётся авторитетным источником, поэтому `failed_polls` ДОЛЖЕН сброситься в 0, а сервер ДОЛЖЕН быть помечен `healthy`. Эта снисходительность не распространяется на сетевые ошибки или не-2xx ответы, которые обрабатываются по FR-26. Она не меняет поведение initial healthcheck (FR-49), который уже помечает explicit-models серверы healthy без опроса. |

### 3.14. Встроенный Web UI (v0.14.0)

| ID     | Требование |
|--------|------------|
| FR-69  | Сам daemon НЕ ДОЛЖЕН отдавать никаких маршрутов Web UI (`GET /ui` и `GET /ui/*` ДОЛЖНЫ возвращать `404`, как и любой другой неопределённый путь). Браузерный dashboard вместо этого ДОЛЖЕН отдаваться отдельной клиентской командой `proxylm web`, по аналогии с `proxylm tui`: она встраивает те же статически собранные ассеты фронтенда (собранные в единый бинарник на этапе компиляции), поднимает собственный локальный HTTP-сервер (`--listen`, по умолчанию `127.0.0.1:8081`) и отдаёт их с `Cache-Control: no-cache` на каждом ответе. `proxylm web` ДОЛЖНА принимать `--connect` (адрес daemon'а `ws://`/`wss://`/`http://`/`https://`, нормализуемый до WebSocket-URL `/admin/stream`), опциональный `--token` (admin-ключ, встраиваемый в отдаваемую страницу для автоподключения) и `--no-open` (не открывать браузер по умолчанию; иначе команда ДОЛЖНА открыть его автоматически после старта локального сервера). Локальный HTTP-сервер команды НЕ ДОЛЖЕН требовать собственной аутентификации — он не отдаёт секретов и живых данных; все живые данные получаются собственным WebSocket-соединением страницы к `/admin/stream` daemon'а, аутентифицируемым по FR-70. |
| FR-70  | Хендшейк WebSocket `/admin/stream` ДОЛЖЕН принимать admin-ключ через дополнительный канал — запись вида `proxylm-token.<base64url-no-padding(admin_key)>` в заголовке запроса `Sec-WebSocket-Protocol` — наряду с существующим механизмом `Authorization: Bearer <admin_key>` (FR-33). Если оба присутствуют в одном запросе, приоритет ДОЛЖЕН быть у `Authorization: Bearer`. Значение admin-ключа, переданное любым из каналов, НИКОГДА не должно писаться в логи. Сервер ДОЛЖЕН согласовывать `proxylm-admin` как выбранный WebSocket-подпротокол; запись с токеном НИКОГДА не должна выбираться как согласованный подпротокол. Существующие TUI-клиенты, которые аутентифицируются только через `Authorization: Bearer` и не предлагают подпротоколов, ДОЛЖНЫ остаться незатронутыми. |
| FR-71  | Встроенный Web UI ДОЛЖЕН быть строго read-only: он НЕ ДОЛЖЕН предоставлять никаких элементов управления, изменяющих состояние daemon'а (без отмены запросов, без правки конфига, без включения/выключения серверов, без управления admin-ключом кроме локального сохранения в браузере для повторного использования). Он ДОЛЖЕН отображать те же категории живого состояния, что и TUI — список серверов (healthy, текущая модель, глубина очереди, порядок по приоритету, perf-метрика), таблицу запросов с TTL-скрытием завершённых/упавших записей по `tui.show_completed_minutes` (FR-36), и детальные представления для выбранного сервера (per-model разрез производительности, FR-41) или запроса (включая хвост генерации, FR-65) — и ДОЛЖЕН автоматически переподключаться с экспоненциальным backoff при разрыве WebSocket, зеркалируя FR-48. |
...
- **Then:** запись имеет `status == "failed"`, `error` содержит описание обрыва. Клиент получил уже отправленные чанки + завершение SSE с error-маркером (см. API.ru.md §1.8).
...
В базовой линии v0.9.3 явно **не** реализуются. Примечание: строго read-only Web UI dashboard, отдаваемый локальной клиентской командой `proxylm web`, а не daemon'ом, появился в v0.14.0 (§3.14, FR-69..FR-71) — это не полноценный read-write веб-интерфейс управления, и перечисленные ниже пункты остаются out-of-scope независимо от него.
...
| ID    | Риск | Митигация |
|-------|------|------------|
| R-1   | Голод модели B при бесконечном потоке модели A (см. ARCHITECTURE.ru.md §3). | Задокументировано как осознанный компромисс стратегии по умолчанию; митигировано (v0.10.0) опциональным лимитом `scheduler.max_consecutive_per_model` под `routing.strategy: fair_share_round_robin` (FR-16). |
...
| R-7   | `modernc.org/sqlite` медленнее, чем CGO-вариант, под высокой нагрузкой на историю. | Запись истории асинхронная (через канал), batch-инсерты; дальнейшая митигация отслеживается в [`docs/FUTURE.ru.md`](./FUTURE.ru.md) (пункт 3). |
...
- **U-4.** При обрыве SSE посреди стрима — отправлять `event: error` с `data: {"error": {"code": "stream_aborted", "message": "..."}}`, затем `data: [DONE]` (см. API.ru.md §1.8).
...
- FR-1 … FR-71 (HTTP API включая Anthropic Messages API, планировщик, маршрутизация, retry, discovery, история, TUI/IPC, CLI, метрики производительности, авто-переподключение, initial healthcheck, X-Request-Id middleware, F5-протокол, dual auth, per-backend protocol, кросс-протокольная трансляция, проба загруженных моделей, сортировка по приоритету, различимые цвета, пульсирующая лампа работы, хвост генерации, персистентность perf-статистики, строгая валидация формы `/v1/models`, read-only Web UI через `proxylm web`)
- NFR-1 … NFR-12
- INV-1 … INV-8
- AC-1 … AC-28
