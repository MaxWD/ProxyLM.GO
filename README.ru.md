...
ProxyLM.GO — это посредник между вашими приложениями и сервисами больших языковых моделей (LLM). Для приложения он выглядит как обычный OpenAI-совместимый сервер, но за его спиной может стоять любое количество фактических серверов — локальные движки (LM Studio, Ollama, vLLM, llama.cpp) и удалённые API (OpenRouter, Groq, Together AI, OpenAI). Главное — чтобы сервер отдавал OpenAI-совместимый `/v1/*` интерфейс; что именно за ним крутится — прокси не задумывается. ProxyLM.GO сам решает, к какому серверу адресовать каждый запрос.
...
- **Несколько бэкендов** — маршрутизация на любое число OpenAI-совместимых серверов; приоритет на бэкенд настраивается (через `priority` можно явно предпочесть локальный сервер облаку, когда оба умеют модель)
