...
1. **Удаление реализованных пунктов.** При каждом значимом релизе (или по запросу) tech-writer проходит файл и **полностью удаляет** блоки, попавшие в выпущенный код. Stub'ы вида «DONE in vX.Y.Z» **не оставляются** — описание реализации уже живёт в `CHANGELOG.md`, `docs/SRS.md`, `docs/ARCHITECTURE.md` и docstring'ах; дублирование в FUTURE.md только создаёт устаревающую копию.
2. **Проверка «не реализован ли пункт походя».** Перед удалением блока tech-writer обязан grep'нуть репозиторий по упомянутым в пункте именам полей / ключей конфига / функций / build-тегов. Если функционал уже есть в коде, пункт удаляется так же, как при намеренной реализации. Это страховка от ситуации, когда реализация прошла по другой задаче, а FUTURE-пункт остался «торчать».
3. **Перенумерация.** После удаления оставшиеся пункты получают **последовательные номера 1..N** (без дырок). Перенумерация затрагивает обе языковые версии (`FUTURE.md` и `FUTURE.ru.md`) в одном коммите.
4. **Внешние ссылки.** Если на удаляемый пункт ссылается другой документ (`SRS.md`, `ARCHITECTURE.md`, `API.md` или CHANGELOG записи прошлых релизов), такие ссылки обновляются: либо на «реализовано в vX.Y.Z», либо на конкретное FR/раздел, который теперь покрывает фичу.
5. **Добавление новых идей.** При появлении новой идеи tech-writer проверяет, что она не дублирует существующий пункт, добавляет блок в конец списка с новым номером и поддерживает единый формат: название, проблема, решение, приоритет, опционально риски/ограничения.
