...
|-----------------------------|-----------------------|------------------------------|
| `docs/ARCHITECTURE.md`      | tech-writer           | внутренняя команда / агенты  |
| `docs/SRS.md`               | tech-writer           | исполнители ТЗ               |
| `docs/API.md`               | tech-writer           | backend + интеграторы        |
| `docs/AGENTS.md` (этот)     | tech-writer           | оркестрация ролей            |
| `docs/FUTURE.md`            | tech-writer           | парковка идей (read-only для исполнителей) |
| `CLAUDE.md` (в корне)       | tech-writer           | Claude Code (AI-ассистент)   |
| `README.md` (в корне)       | go-devops-cli         | внешние пользователи         |
...
**Цель:** поддерживать `docs/ARCHITECTURE.md`, `docs/SRS.md`, `docs/API.md`, `docs/AGENTS.md` и `CLAUDE.md` в актуальном состоянии. Превращать архитектурные изменения в проверяемые требования и в инструкции для AI-ассистента.
...
- Поддерживать `docs/FUTURE.md` — «парковка идей». При каждом релизе/значимом изменении ревизировать список: убирать реализованное, фиксировать новые идеи, всплывшие в ходе работы. Из FUTURE.md **ничего не реализуется самим агентом или Claude Code** — пункты выполняются только по явному запросу пользователя (см. §FUTURE-RULE в CLAUDE.md).

**Формат deliverable:** `docs/SRS.md` + `docs/API.md` + `docs/ARCHITECTURE.md` + `docs/FUTURE.md` + `CLAUDE.md`.
...
- `README.md` (в корне) — описание проекта, скачивание готового бинарника, быстрый старт (3 команды: положить → `./proxylm serve` → `./proxylm tui`), скриншот/мокап TUI, ссылка на `docs/ARCHITECTURE.md` для деталей. Раздел про `service install` под Windows и Linux.
- `go.mod` / зависимости.
