...
### 1.9. Web UI (не эндпоинт daemon'а, v0.14.0)

Сам daemon не отдаёт никаких маршрутов Web UI: `GET /ui` и любой `GET /ui/*` — неопределённые пути и возвращают обычный `404`, как и любой другой неизвестный маршрут.

Read-only браузерный dashboard — это отдельный локальный клиент, команда `proxylm web` (см. `ARCHITECTURE.ru.md` §19), по аналогии с `proxylm tui`. Она встраивает тот же статический фронтенд (`index.html`, `app.js`, `style.css`) в единый бинарник `proxylm`, поднимает собственный локальный HTTP-сервер и отдаёт его с `Cache-Control: no-cache` на каждом ответе:

```bash
proxylm web --connect ws://localhost:8080 --token sk-admin-... --listen 127.0.0.1:8081
```

Это открывает `http://127.0.0.1:8081/` в браузере по умолчанию (если не указан `--no-open`). Команда также отдаёт сгенерированный `/config.js` (`window.PROXYLM = {"ws":"...","token":"..."}`), чтобы страница знала, к какому `/admin/stream` daemon'а подключаться, и, если задан `--token`, подключалась автоматически. `--connect` принимает `ws://`, `wss://`, `http://` или `https://` и нормализует значение до WebSocket-URL `/admin/stream`.

Локальный сервер **не требует собственной аутентификации** — сам по себе он не несёт секретов и живых данных; собственный JavaScript страницы открывает отдельное аутентифицированное WebSocket-соединение к `/admin/stream` daemon'а (§2.1), чтобы получить живое состояние.
...
#### Браузерный канал аутентификации (v0.14.0)

Встроенный Web UI (§1.9) работает в браузере, а нативный браузерный `WebSocket` API не может выставлять заголовки запроса (в частности `Authorization`) на этапе хендшейка. В качестве второго, равнозначного канала аутентификации admin-ключ можно передать через `Sec-WebSocket-Protocol` — одним из значений в списке через запятую:

```
Sec-WebSocket-Protocol: proxylm-admin, proxylm-token.<base64url-no-padding(admin_key)>
```

- Запись-токен имеет фиксированный префикс `proxylm-token.`, за которым следует admin-ключ, закодированный base64url **без padding** (`=`).
- `proxylm-admin` — единственное имя протокола, которое сервер когда-либо согласовывает (эхо в заголовке ответа `Sec-WebSocket-Protocol`); `proxylm-token.<...>` никогда не выбирается как согласованный подпротокол — это лишь побочный канал для ключа.
- Если в одном запросе присутствуют и `Authorization: Bearer`, и запись `proxylm-token.` в подпротоколе, **приоритет у `Authorization: Bearer`**.
- Admin-ключ **никогда не логируется**, независимо от того, каким каналом он передан.
- Существующие TUI-клиенты не затронуты — они аутентифицируются исключительно через `Authorization: Bearer` и не предлагают подпротоколов.
