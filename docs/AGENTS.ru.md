...
- Поддерживать `docs/FUTURE.ru.md` (и его EN-пару `docs/FUTURE.md`) — «парковка идей». При каждом релизе/значимом изменении ревизировать список по §FUTURE-RULE в `CLAUDE.md`. Базовая процедура — **проактивный sweep всех припаркованных пунктов** (а не только тех, что очевидно реализованы в текущем релизе): для каждого пункта grep'нуть репозиторий по упомянутым именам полей / ключей конфига / функций и классифицировать как **полностью реализован** (удалить блок целиком; описание остаётся в CHANGELOG / SRS / ARCHITECTURE — stub в FUTURE не нужен), **частично реализован** (оставить блок, добавив в начало `**Уже сделано в vX.Y.Z:**` с перечнем закрытой части и переписав «Проблема»/«Решение» под остаток; если остаток утратил смысл — удалить так же, как при полной реализации) или **не реализован** (оставить как есть; при необходимости актуализировать формулировку, если архитектура сдвинулась). После sweep'а: перенумеровать оставшиеся пункты последовательно (1..N, без дырок) синхронно в EN и RU версиях в одном коммите, обновить внешние ссылки на номер (`FUTURE.md #N`) — либо на «реализовано в vX.Y.Z», либо на новый номер; новые идеи добавлять в конец списка со следующим номером в едином формате (название / проблема / решение / приоритет / опционально риски). Из FUTURE.ru.md **ничего не реализуется самим агентом или Claude Code** — пункты выполняются только по явному запросу пользователя.
