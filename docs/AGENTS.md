...
- Maintain `docs/FUTURE.md` — the "idea parking lot". At each release or significant change, revise the list per §FUTURE-RULE in `CLAUDE.md`: (1) **delete** blocks that have been implemented (description survives in CHANGELOG / SRS / ARCHITECTURE — no stub here), (2) before deleting — grep the repo for the field / config-key / function names mentioned in the block and remove it even if the feature was implemented incidentally by another task, (3) **renumber** the remaining items sequentially (1..N, no gaps) synchronously in both EN and RU versions, (4) update any external cross-references (`FUTURE.md #N`) to either "implemented in vX.Y.Z" or the new number, (5) add new ideas at the end with the next number, in the standard format (name / problem / solution / priority / optional risks). From FUTURE.md **nothing is implemented by the agent or Claude Code** — items are executed only on explicit user request.
