...
// достаточное, чтобы решать вырожденные случаи». См. FUTURE.md #2 → реализован
// в v0.10.0.
...
// без учёта доверительного интервала. См. FUTURE.md #7 → v0.10.0.
...
// (FUTURE.md #3): запросы разных типов (/v1/chat/completions, /v1/embeddings)
// имеют принципиально разные профили tokens/ms — смешивать их регрессией
// нельзя без искажения оценок.
type perfKey struct {
	server   string
	model    string
	endpoint string
...
//
// endpoint в v0.10.0 стал частью ключа (см. FUTURE.md #3). Пустая строка
// допускается — будет отдельным «безымянным» бакетом (на случай вызова
// до миграции данных в storage / тестового кода).
