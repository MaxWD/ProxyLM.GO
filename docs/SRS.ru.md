...
| FR-41  | Daemon ДОЛЖЕН собирать все наблюдения завершённых запросов по парам `(server_name, model)` в памяти и вычислять параметры линейной модели `loaded·t_load + b·k_in + c·k_out ≈ t_all` (3×3 нормальные уравнения; fallback 2×2 если нет reload-точек или 3×3 сингулярна). Минимум для валидной оценки — 3 наблюдения (`perfMinSamples = 3`). Поля `perf_ok`, `t_load_ms`, `tok_in_per_sec`, `tok_out_per_sec` публикуются в `ServerState`; TUI ДОЛЖЕН показывать их в строке HeaderBar текущей модели сервера в формате `t_load · ↓tok_in/s · ↑tok_out/s`. Если `perf_ok = false` или `current_model` пусто — строка метрик пустая. Поля `tokens_per_sec` и `ttft_ms` **удалены**. |
