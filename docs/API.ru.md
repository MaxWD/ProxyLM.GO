...
### 1.9. `GET /ui` и `/ui/*` (встроенный Web UI, v0.14.0)

`GET /ui` возвращает `301 Moved Permanently` на `/ui/`. `GET /ui/*` отдаёт встроенный статический фронтенд (`index.html`, `app.js`, `style.css`) — read-only дашборд, зеркалирующий TUI (см. `ARCHITECTURE.ru.md` §19).

Эти пути **не требуют аутентификации** — сами по себе они не несут секретов и живых данных; собственный JavaScript страницы открывает отдельное аутентифицированное WebSocket-соединение к `/admin/stream` (§2.1), чтобы получить живое состояние. Каждый ответ содержит `Cache-Control: no-cache`, чтобы пересобранный бинарник никогда не отдавал устаревший фронтенд из кэша браузера.

```bash
curl -s http://localhost:8080/ui/          # вернёт index.html
